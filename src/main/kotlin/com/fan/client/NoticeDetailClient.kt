...
object NoticeDetailClient {
    ...
    fun getDetailPageUrl(
        stock: String,
        code: String,
    ): String = String.format(DETAIL_PAGE_URL, stock, code)
     ...
     * 提取股票代码
     */
    private fun extractStock(
        detail: WebNoticeDetailResponse,
        stock: String,
    ): String =
        detail.data
            ?.security
            ?.first { it.stock == stock }
            ?.stock
            ?: detail.data
                ?.security
                ?.first { it.stock.length == 6 }
                ?.stock
                .orEmpty()
     ...
     * 格式化HTML为纯文本
     */
    private fun formatHtml2Text(html: String): String = html
}
